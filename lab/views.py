...
from .serializers import SupplierFormSerializer,SupplierSerializer, CustomerSerializer, LaboratoryFormSerializer
...
def get_lab_forms(request, lab_id):
    ...
    try:
        ...
        forms = SupplierForm.objects.filter(laboratory=lab,status='Pending') # get all forms sent to that lab_id

        serializer = SupplierFormSerializer(forms, many=True, context={'request': request}) # returns an array of objects
...
def update_form_status(request, lab_id, supplier_id):
    try:
        ...
        if form.status == 'Verified':
            return Response({"error": "Form is already verified."}, status=status.HTTP_400_BAD_REQUEST)
            
        if form.status == 'Rejected':
            return Response({"error": "Form is Rejected."}, status=status.HTTP_400_BAD_REQUEST)
